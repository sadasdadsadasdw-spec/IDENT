# Install IDENT -> Bitrix24 Integration to Windows Scheduled Task
...
$ExePath = Join-Path $ScriptDir "dist\ident_sync.exe"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Installing IDENT Integration" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $ExePath)) {
    ...
    Write-Host ""
    Write-Host "Build first: .\build_exe.ps1" -ForegroundColor Yellow
    Write-Host ""
    Read-Host "Press Enter to exit"
    exit 1
}
...
# Task action - SINGLE action only
$Action = New-ScheduledTaskAction `
    -Execute $ExePath `
    -WorkingDirectory $ScriptDir

# Task trigger - SINGLE trigger: at system startup
$Trigger = New-ScheduledTaskTrigger -AtStartup

# Task settings - NO restart on failure to prevent double spawning
...
if ($Response -eq 'y') {
    ...
    if ($Processes) {
        ...
        if ($ProcessCount -gt 1) {
            ...
            Write-Host "This should not happen. Check task configuration." -ForegroundColor Yellow
        }
    } else {
